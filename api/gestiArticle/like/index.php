<?php
    ...
    switch ($http_method) {
        ...
        case "POST":
            try {
                ...
                if (!isset($postedData['id_user'])) throw new Exception("[". API_NAME ."] ". $http_method ." request : Erreur vous devez donner une clé id_user.");
                if (!isset($postedData['id_post'])) throw new Exception("[". API_NAME ."] ". $http_method ." request : Erreur vous devez donner une clé id_post.");
                ...
                $id_user = htmlspecialchars($postedData['id_user'], ENT_QUOTES, 'UTF-8');
                $id_post = htmlspecialchars($postedData['id_post'], ENT_QUOTES, 'UTF-8');
                ...
                if (is_id_in($id_user, $id_post, $db)) throw new Exception("[". API_NAME ."] ". $http_method ." request : Erreur vous avez déjà liké ou disliké cette article. (Donner une clé id_user et id_post n'existant pas dans la base de données)");
        ...
        case "PUT":
            try {
                ...
                $postedData = $json_data;
                if (!isset($postedData['id_user']) || !isset($postedData['id_post']) || !isset($postedData['is_like'])) {
                    throw new Exception("[". API_NAME ."] ". $http_method ." request : Erreur vous devez donner les clés suivantes : id_user, id_post, is_like.");
                }
                ...
                /// Vérification de l'appartenance de l'utilisateur à l'appreciations
                if(!is_owner($bearer_token, $id_user)) {
                    deliver_response(403, "[". API_NAME ."] ". $http_method ." request : Erreur vous n'avez pas les droits pour accéder à ce like.", NULL);
                    exit;
                }

                /// Vérification de l'existence de l'id_user et id_post dans la base de données
                if (!is_id_in($id_user, $id_post, $db)) throw new Exception("[". API_NAME ."] ". $http_method ." request : Erreur vous devez donner une clé id_user et id_post existant dans la base de données.");
        ...
        case "DELETE":
            try {
                ...
                $postedData = $json_data;
                if (!isset($postedData['id_user']) || !isset($postedData['id_post'])) {
                    throw new Exception("[". API_NAME ."] ". $http_method ." request : Erreur vous devez donner les clés suivantes : id_user, id_post.");
                }
                ...
                /// Vérification de l'appartenance de l'utilisateur au like
                if(!is_owner($bearer_token, $id_user)) {
                    deliver_response(403, "[". API_NAME ."] ". $http_method ." request : Erreur vous n'avez pas les droits pour accéder à cette article.", NULL);
                    exit;
                }
    ...
    /**
     * Vérifie si le like appartient à l'utilisateur
     * @param string $token Le token JWT
     * @param PDO $db Le lien de connexion à la base de données
     * @return bool Vrai si le l'appreciations appartient à l'utilisateur, faux sinon
     * @author Christopher ASIN <https://github.com/RiperPro03>
     */
    function is_owner($token, $id_user) {
        if (get_id_token($token) == $id_user) {
            return true;
        } else {
            return false;
        }
