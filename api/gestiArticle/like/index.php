<?php
    ...
    switch ($http_method) {
        ...
        case "GET":
            ...
            try {
                if (!empty($_GET['id'])) {
                    ...
                    $q = $db->prepare("SELECT SUM(is_like) as nb_like, SUM(CASE WHEN is_like = 0 THEN 1 ELSE 0 END) AS nb_dislike FROM aimer");
                    $q->execute();
        ...
        case "POST":
            try {
                ...
                $matchingData = lastData($db, "aimer");
        ...
        //             throw new Exception("[". API_NAME ."] ". $http_method ." request : Erreur vous devez donner au moins une des clés suivantes : id_user, id_post.");
        //         }
                
        //         //Eviter la faille XSS
        //         $id_user = isset($postedData['id_user']) ? htmlspecialchars($postedData['id_user'], ENT_QUOTES, 'UTF-8') : null;
        //         $id_post = isset($postedData['id_post']) ? htmlspecialchars($postedData['id_post'], ENT_QUOTES, 'UTF-8') : null;
        //         $is_like = isset($postedData['is_like']) ? htmlspecialchars($postedData['is_like'], ENT_QUOTES, 'UTF-8') : null;
        ...
        case "PUT":
            try {
                ...
                if (!empty($_GET['id'])) {
                    ...
                    if (is_Id($id, $db)) {
                        /// Vérification de l'appartenance de l'utilisateur au post
                        if(!is_owner($bearer_token, $db, $id_post) && get_role_token($bearer_token) === "publisher") {
    ...
    function is_owner($token, $db, $id_post) {
        $q = $db->prepare("SELECT * FROM posts WHERE id_user = :id_user AND id_post = :id_post");
